...
export async function setOpenrouterApiKey(apiKey: string): Promise<void> {
  const clientId = await getClientId();
  await db.saveApiKeys(clientId, { openrouterApiKey: apiKey });
}

export async function clearOpenrouterApiKey(): Promise<void> {
  const clientId = await getClientId();
  await db.saveApiKeys(clientId, { openrouterApiKey: undefined });
}

export async function getAiApiKey(): Promise<
  { provider: 'groq' | 'anthropic' | 'openrouter'; key: string } | undefined
> {
  const openrouterKey = await getOpenrouterApiKey();
  if (openrouterKey?.trim()) {
    return { provider: 'openrouter', key: openrouterKey.trim() };
  }
...
export async function getAiKeyInfo(): Promise<{
  ...
  const openrouterKey = keys?.openrouterApiKey?.trim();
  if (openrouterKey) {
    return {
      configured: true,
      provider: 'openrouter',
  ...
  if (process.env.OPENROUTER_API_KEY?.trim()) {
    return {
      ...
      provider: 'openrouter',
      source: 'env',
      maskedKey: maskApiKey(process.env.OPENROUTER_API_KEY),
    };
  }
...
export async function setAiApiKey(apiKey: string): Promise<void> {
  ...
  if (trimmed.startsWith('sk-or-') || (!trimmed.startsWith('gsk_') && !trimmed.startsWith('sk-ant-'))) {
  ...
  throw new Error('Invalid API key format. Use an OpenRouter (sk-or-...), Groq (gsk_...), or Anthropic (sk-ant-...) key.');
}
